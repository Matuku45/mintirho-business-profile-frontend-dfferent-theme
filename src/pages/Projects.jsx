...
const Projects = () => {
return ( <main className="min-h-screen bg-gradient-to-br from-gray-100 to-gray-50 px-6 py-20"> <div className="max-w-7xl mx-auto text-center"> <h1 className="text-5xl font-bold mb-12 text-gray-800">Our Projects</h1> <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
{projectData.map((project, index) => ( <div  
           key={index}  
           className="bg-white rounded-xl shadow-2xl overflow-hidden hover:scale-105 transform transition-all duration-500 hover:shadow-3xl"  
         > <img src={project.image} alt={project.title} className="w-full h-64 object-cover transition-transform duration-500 hover:scale-110" /> <div className="p-6 text-left"> <h3 className="text-2xl font-bold text-gray-800 mb-3">{project.title}</h3> <p className="text-gray-600 mb-2">{project.description}</p> <p className="text-gray-700 mb-1"><span className="font-semibold">Client:</span> {project.client}</p> <p className="text-gray-700 mb-1"><span className="font-semibold">Contact Person:</span> {project.contactPerson}</p> <p className="text-gray-700 mb-1"><span className="font-semibold">Contact Number:</span> {project.contactNumber}</p> <p className="text-gray-700 mb-1"><span className="font-semibold">Contract Amount:</span> {project.contractAmount}</p> <p className="text-gray-700 mb-1"><span className="font-semibold">Contract Period:</span> {project.contractPeriod}</p> <p className="text-gray-700 mb-1"><span className="font-semibold">Start Date:</span> {project.startDate}</p> <p className="text-gray-700 mb-1"><span className="font-semibold">Scheduled Completion:</span> {project.scheduleCompletion}</p> <p className="text-gray-700 mb-3"><span className="font-semibold">Actual Completion:</span> {project.actualCompletion}</p>
<span
className={`inline-block px-3 py-1 text-sm font-semibold rounded-full ${project.status === 'Ongoing' ? 'bg-yellow-200 text-yellow-800' : 'bg-green-200 text-green-800'}`}
>
